...
!_PROC_EXPORT(open_)
   subroutine open_( unit, file, form, status, action, access, recl, iostat, descr )
      ...
      elseif (iostat_ /= 0) then
         ...
         if (reopened) then; write(buffer, 100, err=20) iostat_, trim(descr_), trim(file), trim(form_), trim(status_)
         else; write(buffer, 200, err=20) iostat_, trim(descr_), trim(file), trim(form_), trim(status_)
         end if

         20 call throw( IOError, buffer )
      end if

      100 format( 'code ', I2, ', can not reopen ', A, ' "', A, '" in mode (', A, ', ', A, ')' )
      200 format( 'code ', I2, ' on opening ', A, ' "', A, '" in mode (', A, ', ', A, ')' )
   end subroutine
...
!_PROC_EXPORT(close_)
   subroutine close_( unit, status, iostat, descr )
      ...
      elseif (iostat_ /= 0) then
         ! we handle error by throwing exception ...
         _optArgPtr(descr_, descr, descr_std)
         inquire( unit, name=file )
         write(buffer, 99, err=20) iostat_, trim(descr_), trim(file), trim(status_)
