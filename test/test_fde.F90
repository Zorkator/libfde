...
contains
  ...
  subroutine cleanup_basedata()!{{{
    ...
    p_hashmap => getScope()
    call delete( p_hashmap )
    deallocate( p_hashmap )
  end subroutine!}}}
end module!}}}
...
#   define _map_ref( typeId ) \
  ...
  subroutine test_hashmap_nesting()!{{{
    ...
    print *, connectCallback( scope, 'myCB', callback_proc_ )
    call invokeCallback( scope, 'myCB', c_loc(scope) )

    if (dynamic_cast( r_array, get(scope, 'value') )) &
      print *, r_array

    if (localize( getScope(), 'gcsm', item_hit_ )) then
      print *, "found"
    end if
  ...
  logical &!{{{
  function item_hit_( item ) result(res)
    type(Item_t), target :: item
    type(HashMap_t), pointer :: ptr
    res = dynamic_cast( ptr, mk_item_ptr_(item) )
  end function!}}}

  function mk_item_ptr_( item ) result(res)!{{{
    type(Item_t), target  :: item
...
!_PROC_EXPORT(streamvisitor_create)
  ...
  subroutine test_binstreamvisitor()
    use mod_basedata
...
module mod_pointer_remapping!{{{
  ...
  public :: test_pointer_remapping
  public :: test_pointer_remapping_buf
...
contains
  ...
  subroutine test_pointer_remapping()!{{{
    real*4, dimension(:,:,:), pointer :: ptr
  ...
  subroutine test_pointer_remapping_buf()
    allocate( buffer(20) )
...
program test_fde
  use mod_basedata
  use mod_basic
  use mod_binstreamvisitor
  use mod_pointer_remapping

  call init_basedata()
    call test_basic()
    call test_binstreamvisitor()
    call test_pointer_remapping
    call test_pointer_remapping_buf
